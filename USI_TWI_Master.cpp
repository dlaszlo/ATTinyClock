		...
		unsigned char msgSize)
{
	...
	/* Release SCL to ensure that (repeated) Start can be performed */PORT_USI |=
			(1 << PIN_USI_SCL);                     // Release SCL.
	...
	/* Generate Start Condition */PORT_USI &= ~(1 << PIN_USI_SDA); // Force SDA LOW.
	_delay_us(T4_TWI);
	...
	do
	{
		...
		if (USI_TWI_state.addressMode || USI_TWI_state.masterWriteDataMode)
		{
			...
			/* Clock and verify (N)ACK from slave */DDR_USI &= ~(1
					<< PIN_USI_SDA);                // Enable SDA as input.
